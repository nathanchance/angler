...
static void cpufreq_flash_boost(struct cpufreq_flash_tunables *tunables)
{
	int i;
	int anyboost = 0;
...
static ssize_t show_boost(struct cpufreq_flash_tunables *tunables,
			  char *buf)
{
	return sprintf(buf, "%d\n", tunables->boost_val);
}

static ssize_t store_boost(struct cpufreq_flash_tunables *tunables,
			   const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (tunables->boost_val) {
		trace_cpufreq_flash_boost("on");
		if (!tunables->boosted)
			cpufreq_flash_boost(tunables);
	} else {
		tunables->boostpulse_endtime = ktime_to_us(ktime_get());
		trace_cpufreq_flash_unboost("off");
	}

	return count;
}

static ssize_t store_boostpulse(struct cpufreq_flash_tunables *tunables,
				const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	trace_cpufreq_flash_boost("pulse");
	if (!tunables->boosted)
		cpufreq_flash_boost(tunables);
	return count;
}

static ssize_t show_boostpulse_duration(struct cpufreq_flash_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%d\n", tunables->boostpulse_duration_val);
}

static ssize_t store_boostpulse_duration(struct cpufreq_flash_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
