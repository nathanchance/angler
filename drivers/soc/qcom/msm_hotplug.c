...
static void big_updown(unsigned int fast_lane_req)
{
    ...
    online_big = target_big = num_online_big_cpus();
    ...
    if (online_big > 0) {
        ...
        avg_load_little_cpus = load_at_max_freq() / online_little;
        avg_load_big_cpus = 0;
    }
    ...
    } else if (online_little == LITTLE_CORES) {
        // Only if all little are up, up big cores depending on load, one at a time
        if ((avg_load_little_cpus >= hotplug.kick_in_load_big && online_big == 0   ) ||
            (avg_load_big_cpus >= hotplug.online_load_big && online_big < BIG_CORES)    ) {
...
static DEVICE_ATTR(update_rates, (S_IWUGO|S_IRUGO), show_update_rates, store_update_rates);
